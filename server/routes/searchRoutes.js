...
router.get("/search", async (req, res) => {
  try {
    ...
    if (totalCount === 0) {
      return res
        .status(404)
        .json({ message: "No applications found matching the criteria." });
    }
    ...
    res.json({
      ...
      pagination: {
        ...
        nextOffset:
          pageOffset + pageLimit < totalCount ? pageOffset + pageLimit : null,
...
router.get("/top-blocks", async (req, res) => {
  ...
  COUNT(*) AS count
FROM 
  pp.applicant_primary_info a
JOIN 
  pp.jurisdiction j
  ON a.nmms_block = j.juris_code
GROUP BY 
  j.juris_name, a.nmms_block
ORDER BY 
  count DESC
LIMIT 5;
    `;
...
router.get("/shortlist-top-blocks", async (req, res) => {
  try {
    const query = `
     SELECT 
  j.juris_name,
  COUNT(DISTINCT a.applicant_id) AS count
FROM 
  pp.applicant_primary_info a
JOIN 
  pp.jurisdiction j
